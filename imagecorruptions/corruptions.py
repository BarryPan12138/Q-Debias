...
def gaussian_noise(x, severity=1):
    c = [0.08, 0.15, 0.22, 0.35, 0.50][severity - 1]
...
def impulse_noise(x, severity=1):
    c = [.03, .12, .20, 0.35, 0.5][severity - 1]
...
def glass_blur(x, severity=1):
    ...
    c = [(0.7, 1, 2), (0.9, 2, 1), (1, 2, 3), (2, 4, 2), (4, 8, 4), (6, 10, 6)][
        severity - 1]
...
def defocus_blur(x, severity=1):
    c = [(6, 0.5), (9, 0.5), (10, 0.5), (15, 0.5), (20, 0.7)][severity - 1]
...
def motion_blur(x, severity=1):
    ...
    c = [(10, 3), (15, 8), (20, 12), (25, 15), (40, 25)][severity - 1]
    x = np.array(x)
...
def jpeg2k_compression(x, severity=1):
    c = [25, 18, 15, 10, 7][severity - 1]

    img_cv = np.array(x)
    img_cv = cv2.cvtColor(img_cv, cv2.COLOR_RGB2BGR)

    encode_param = [cv2.IMWRITE_JPEG2000_COMPRESSION_X1000, c]
    success, encoded_img = cv2.imencode('.jp2', img_cv, encode_param)

    if not success:
        raise ValueError("JPEG 2000 压缩失败")
